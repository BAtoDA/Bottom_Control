...
namespace 自定义Uppercomputer_20200727
{
    public partial class keyboard : Skin_VS
    {
        ...
        public keyboard(string Name,DATextBox textBox) 
        {
            InitializeComponent();
        ...
        private void keyboard_Shown(object sender, EventArgs e)//加载控件状态
        {
            ...
            if (textBox.numerical.ToString() == "Float_32_Bit") this.skinButton4.Enabled = true;//允许输入小数点
            if (textBox.numerical.ToString() == "Unsigned_16_Bit" || textBox.numerical.ToString() == "Unsigned_16_Bit") this.skinButton9.Enabled = false;//不允许输入符号
        ...
        private void Button_KeyPress(object sender, EventArgs e)//获取用户输入
        {
            ...
            {
                if(textBox.numerical.ToString() == "Float_32_Bit") this.skinTextBox3.Text = complement(this.skinTextBox3.Text);//自动补码小数点
                this.Close();//关闭窗口
            }
            if ((button.Text != "Clr") & (button.Text != "Esc") & (button.Text != "Bs") & (button.Text != "Del") & (button.Text != "<") & (button.Text != ">") & (button.Text != "Enter"))//判断是否功能键不录入
            {
                if ((textBox.numerical.ToString() == "Binary_16_Bit"|| textBox.numerical.ToString() == "Binary_32_Bit")&(button.Text.ToInt32()>1)) return;//返回方法不录入字体
        ...
        private bool numerical_KeyPress_import(string Text, string Name)//获取用户输入的文本-与参数中的格式-判断输入是否正确
        {
            ...
            if ((textBox.numerical.ToString() != "Float_32_Bit") &(Text.IndexOf('.') > 0)) Text.Trim('.');//如果不浮点小数 就移除小数点
            if ((textBox.numerical.ToString() == "Unsigned_16_Bit" || textBox.numerical.ToString() == "Unsigned_32_Bit") & (Text.IndexOf('-') > 0)) Text.Trim('-');//如果不是有符号就移除符号
            ...
            switch (Name.Trim())//判断数据
            {
                ...
                case "Float_32_Bit":
                    ...
                    Handled =  In> textBox.Decimal_Below ? true:false;//取消修改
                    break;
        ...
        private string[] Constraints_data(string Name)//实现约束最小值--最大值
        {
            ...
            switch(Name)
            {
                ...
                case "Signed_16_Bit":
                    ...
                    //最小值
                    data[1] = "-";//先填充符号
                    if(textBox.Decimal_Below >= 5)
                        data[1] = "-32766";//大于限制默认填充最大值
                    else
                        for (int i = 0; i < textBox.Decimal_Below; i++)//先填充最大值
                        {
                            data[1] += "9";
                        }
                    break;
