...
namespace Bottom_Control.基本控件
{
    //==============================================================
    //  作者：BAtoDA
    //  时间：2021/2/20 15:30:04 
    //  文件名：DASignalLamp 
    //  版本：V1.0.1  
    //  说明：  实现上位机底层控件 指示灯带闪烁类 -不再公共运行时 
    //  修改者：***
    //  修改说明： 
    //==============================================================
    /// <summary>
    /// 实现上位机底层控件 指示灯带闪烁类 -不再公共运行时
    /// </summary>
    [ToolboxItem(true)]
    [Browsable(true)]
    [Description("实现上位机底层控件 指示灯带闪烁类 -不再公共运行时")]
    public class DASignalLamp: UCSignalLamp, Button_base, Flicker_base
    {
        #region 实现接口参数
        ...
        #endregion
        #region 实现闪烁接口参数
        [Description("值为0时  是否开启闪烁功能"), Category("PLC类型-闪烁参数")]
        [DefaultValue(typeof(bool), "false")]
        public bool O_Flicker { get; set; } = false;
        [Description("值为0时 闪烁颜色变换队列"), Category("PLC类型-闪烁参数")]
        public Color[] O_FlickerColor { get; set; } = new Color[] { Color.FromArgb(255, 77, 59) };
        [Description("值为0时 闪烁颜色变换时间"), Category("PLC类型-闪烁参数")]
        public int O_FlickerTime { get; set; } = 1000;
        [Description("值为1时  是否开启闪烁功能"), Category("PLC类型-闪烁参数")]
        [DefaultValue(typeof(bool), "false")]
        public bool I_Flicker { get; set; } = false;
        [Description("值为1时 闪烁颜色变换队列"), Category("PLC类型-闪烁参数")]
        public Color[] I_FlickerColor { get; set; } = new Color[] { Color.FromArgb(255, 77, 59) };
        [Description("值为1时 闪烁颜色变换时间"), Category("PLC类型-闪烁参数")]
        public int I_FlickerTime { get; set; } = 1000;
        #endregion
        public DASignalLamp()
        {
            plc = new Button_PLC();
